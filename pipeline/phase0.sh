...
SCRIPT=`dirname $GINGER_PATH`/../util/merge_phase0
CDIR=`pwd`

PREFIX=${1}

if test $# -ne 1 ; then
    echo "--- GINGER pipeline; phase0 ---"
    #    echo "1.prefix"
    echo "1.prefix"
else
    #import config
    echo "perl ${SCRIPT}/genConfigIniForMerge.pl ${CDIR}/output/ ${PREFIX} > config.ini"
    perl ${SCRIPT}/genConfigIniForMerge.pl ${CDIR}/output/ ${PREFIX} > config.ini
