...
SCRIPT=/data2/fumiya/mytools/Lemon/newLemon_v2.1/Prep/homology/homologyscript
...
if [ $# -ne 5 ]; then
	...
	echo "./homology.sh [genome] [related species protein] [spalndatabase] [thread] [prefix]" 1>&2
	exit 1
fi
...
outputspalnfilter2=$6"_spalnresult_over90.gff"
...
spaln -Q7 -O4 -ospalnresult_o4 -M -yS# -T$3 -yB# -yZ -t$4 -drefer relate.faa 
mv spalnresult_o4 ${5}_spalnresult_alignment.tsv

#gff作成
python $SCRIPT/o4_to_gff.py ${5}_spalnresult_alignment.tsv $5 > ${5}_spalnresult.gff
...
rm refer.mfa relate.faa fastarepair
...
#awk '$2<90' alignmentresult.txt | awk '{print $1}' > alignmentfilterlist2.txt

#./exclude alignmentfilterlist.txt spaln_filter_flameshift.gff $outputspalnfilter
#./exclude alignmentfilterlist2.txt spaln_filter_flameshift.gff $outputspalnfilter2

#削除
#rm flameshiftlist.txt alignmentfilterlist.txt alignmentfilterlist2.txt
#rm flameshiftfilter prefilter01.fa exclude spaln_filter_flameshift.gff function.hpp
#rm prefilter01.fa function.hpp

#mv alignmentresult.txt ${6}_alignmentresult.txt
