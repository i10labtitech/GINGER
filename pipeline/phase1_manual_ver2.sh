...
filter=${SCRIPT}/score_filtering_ver2.py
...
if test $# -ne 6 ; then
    ...
    echo "5: score threshold (for single exon gene)"
    echo "6: score threshold (for multi exon gene)"
else
    ...
    python $filter ${prefix}_raw.gff Group_sub.gff $n1 $n2 ${prefix}_filter.gff
