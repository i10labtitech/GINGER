...
my %w             = ();
my %additionalGff = ();
&loadData($configFile, \$baseDir, \$oPrefix, \%w, \%additionalGff);

##################################################
# Output for config.ini
##################################################

my $cwd = `pwd`; # added for tests
chomp($cwd);     # added for tests

my $mappingGtf     = "$cwd/$oPrefix.gtf";                            # modified for tests
my $mappingToMerge = "$cwd/$oPrefix\_merge.gff3";                    # modified for tests
my $gmapResult     = "$cwd/$oPrefix\_denovo.cd.fasta.gmap.filtered"; # modified for tests
my $denovoResult   = "$cwd/$oPrefix\_final.gff3";                    # modified for tests
my $homologyResult = "$cwd/all_homology_filter.gff";                 # modified for tests
my $augustusResult = "$cwd/Augustus_abinitio.gtf";                   # modified for tests
my $snapResult     = "$cwd/$oPrefix.final.gff";                      # modified for tests 
...
sub loadData {
    my($configFile, $baseDir, $oPrefix, $w, $additionalGff) = @_;
